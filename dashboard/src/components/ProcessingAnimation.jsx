...
const ProcessingAnimation = ({ media, isComplete, syncedTime, isSyncedPlaying, syncTrigger }) => {
  ...
  const containerClasses = `relative w-full aspect-video rounded-xl overflow-hidden bg-ink border border-line shadow-xl mb-8 group animate-[fadeIn_0.5s_ease-out] transition-all duration-500
    ${isComplete && !isSyncedPlaying ? 'grayscale brightness-50' : ''}
    ${isSyncedPlaying ? 'ring-2 ring-primary ring-offset-2 ring-offset-background shadow-primary/20' : ''}`;
  ...
  return (
    <div className={containerClasses}>
      ...
      <div className={`absolute inset-0 transition-all duration-700 ${getVideoOpacityClass()}`}>
        {isYouTube && videoSrc ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-ink">
             <div className="w-16 h-16 border-4 border-white/10 border-t-white/40 rounded-full animate-spin"></div>
          </div>
        )}
      </div>
      ...
      {!isSyncedPlaying && !isComplete && (
        <>
            <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.04)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.04)_1px,transparent_1px)] bg-[size:40px_40px] z-10 pointer-events-none"></div>
            <div className="absolute left-0 w-full h-[2px] bg-accent shadow-[0_0_15px_2px_rgba(122,92,78,0.55)] animate-[scan_2.5s_linear_infinite] z-20 pointer-events-none"></div>
            <div className="absolute left-0 w-full h-[15%] bg-gradient-to-b from-accent/0 via-accent/10 to-accent/0 animate-[scan-overlay_2.5s_linear_infinite] z-10 pointer-events-none"></div>
        </>
      )}

      {/* HUD Elements - Hide when synced playing */}
      {!isSyncedPlaying && (
          <div className={`absolute top-4 left-4 z-30 flex items-center gap-2 px-3 py-1.5 backdrop-blur-md rounded-lg border text-xs font-mono font-bold uppercase transition-all duration-500 ${isComplete ? 'bg-green-500/20 border-green-400/30 text-green-300' : 'bg-black/60 border-accent/40 text-[#E7C9B8] animate-pulse'}`}>
            {isComplete ? (
                <>
      ...
      {!isSyncedPlaying && !isComplete && (
          <div className="absolute inset-0 pointer-events-none z-20 overflow-hidden">
             <div className="absolute top-0 bottom-0 left-[35%] w-[1px] bg-accent/25 border-r border-dashed border-accent/50"></div>
             <div className="absolute top-0 bottom-0 right-[35%] w-[1px] bg-accent/25 border-l border-dashed border-accent/50"></div>
             <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 border border-white/20 rounded-full flex items-center justify-center">
                <div className="w-1 h-1 bg-[#E7C9B8] rounded-full animate-ping"></div>
             </div>
       ...
       {/* Bottom Info Bar */}
      {!isSyncedPlaying && !isComplete && (
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/90 to-transparent z-30 flex justify-between items-end border-t border-white/10">
              <div className="font-mono text-[10px] text-[#E7C9B8] space-y-1">
              ...
              <div className="flex gap-1">
                 <div className="w-1 h-3 bg-[#E7C9B8]/40 animate-[pulse_0.5s_infinite]"></div>
                 <div className="w-1 h-5 bg-[#E7C9B8]/60 animate-[pulse_0.7s_infinite]"></div>
                 <div className="w-1 h-2 bg-[#E7C9B8]/30 animate-[pulse_0.4s_infinite]"></div>
                 <div className="w-1 h-4 bg-[#E7C9B8]/80 animate-[pulse_0.6s_infinite]"></div>
                 <div className="w-1 h-3 bg-[#E7C9B8]/50 animate-[pulse_0.5s_infinite]"></div>
              </div>
          </div>
      )}
    </div>
