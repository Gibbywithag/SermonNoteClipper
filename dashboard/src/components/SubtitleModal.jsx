...
export default function SubtitleModal({ isOpen, onClose, onGenerate, isProcessing, videoUrl, jobId, clipIndex, existingHook }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
            <div className="bg-surface border border-line p-6 rounded-2xl w-full max-w-5xl shadow-2xl relative flex flex-col md:flex-row gap-6 max-h-[90vh]">
                <button
                    onClick={onClose}
                ...
                <div className="w-full md:w-80 flex flex-col">
                    <h3 className="font-display italic text-2xl text-ink mb-4 flex items-center gap-2 shrink-0">
                    ...
                    <div className="space-y-5 flex-1 overflow-y-auto custom-scrollbar pr-1">
                        {/* Position Selector */}
                        <div>
                            <label className="text-xs font-bold text-muted uppercase tracking-wider mb-2 block">Position</label>
                            <div className="grid grid-cols-3 gap-2">
                                {['top', 'middle', 'bottom'].map((pos) => (
                                    <button
                                        key={pos}
                                        onClick={() => setPosition(pos)}
                                        className={`p-2 rounded-lg border text-center text-xs font-medium transition-all ${position === pos ? 'bg-primary/15 border-primary text-ink' : 'bg-stone border-line text-muted hover:bg-line/60'}`}
                                    >
                        ...
                        {/* Animation Style (new) */}
                        <div>
                            <label className="text-xs font-bold text-muted uppercase tracking-wider mb-2 block">Animation</label>
                            <div className="grid grid-cols-2 gap-2">
                                {ANIMATION_OPTIONS.map((opt) => (
                                    <button
                                        key={opt.value}
                                        onClick={() => setAnimation(opt.value)}
                                        className={`p-2 rounded-lg border text-center text-xs font-medium transition-all ${animation === opt.value ? 'bg-primary/15 border-primary text-ink' : 'bg-stone border-line text-muted hover:bg-line/60'}`}
                                    >
                                        {opt.label}
                                    </button>
                                ))}
                        ...
                        {useRemotionPreview && (
                            <div>
                                <button
                                    ...
                                    className="w-full flex items-center justify-between text-xs font-bold text-muted uppercase tracking-wider mb-2"
                                >
                                ...
                                {showTextEditor && (
                                    <textarea
                                        ...
                                        rows={5}
                                        className="w-full bg-background border border-line rounded-lg p-2.5 text-sm text-ink focus:outline-none focus:border-primary/50 resize-none leading-relaxed animate-[fadeIn_0.15s_ease-out]"
                        ...
                        {/* Font Family */}
                        <div>
                            <label className="text-xs font-bold text-muted uppercase tracking-wider mb-2 block">Font</label>
                            <select
                                value={fontName}
                                onChange={(e) => setFontName(e.target.value)}
                                className="w-full bg-background border border-line rounded-lg p-2 text-sm text-ink focus:outline-none focus:border-primary/50"
                            >
                                {FONT_OPTIONS.map((f) => (
                        ...
                        {/* Text Color */}
                        <div>
                            <label className="text-xs font-bold text-muted uppercase tracking-wider mb-2 block">Text Color</label>
                            <div className="flex flex-wrap gap-2">
                                {COLOR_PRESETS.map((c) => (
                                    <button
                                        key={c.color}
                                        onClick={() => setFontColor(c.color)}
                                        className={`w-7 h-7 rounded-full border-2 transition-all ${fontColor === c.color ? 'border-ink scale-110' : 'border-line hover:border-muted'}`}
                                        style={{ backgroundColor: c.color }}
                                        title={c.label}
                                    />
                                ))}
                                <label className="w-7 h-7 rounded-full border-2 border-dashed border-line cursor-pointer flex items-center justify-center hover:border-muted transition-all overflow-hidden relative" title="Custom color">
                        ...
                        {/* Highlight Color (new) */}
                        <div>
                            <label className="text-xs font-bold text-muted uppercase tracking-wider mb-2 block">Highlight Color</label>
                            <div className="flex flex-wrap gap-2">
                                {[{ color: '#FFDD00', label: 'Gold' }, { color: '#FF4444', label: 'Red' }, { color: '#00FF88', label: 'Green' }, { color: '#00BBFF', label: 'Blue' }, { color: '#FF69B4', label: 'Pink' }].map((c) => (
                                    <button
                                        ...
                                        className={`w-7 h-7 rounded-full border-2 transition-all ${highlightColor === c.color ? 'border-ink scale-110' : 'border-line hover:border-muted'}`}
                                        style={{ backgroundColor: c.color }}
                                        title={c.label}
                                    />
                                ))}
                        ...
                        {/* Border / Outline */}
                        <div>
                            <label className="text-xs font-bold text-muted uppercase tracking-wider mb-2 block">Border</label>
                            <div className="flex items-center gap-3">
                                <label className="relative w-8 h-8 rounded-lg border border-line cursor-pointer overflow-hidden shrink-0" title="Border color">
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-2">
                                <label className="text-xs font-bold text-muted uppercase tracking-wider">Background Box</label>
                                <label className="relative inline-flex items-center cursor-pointer">
                                    ...
                                    <div className="w-8 h-4 bg-line peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[0px] after:left-[0px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-primary"></div>
                                </label>
                            </div>
                            {bgOpacity > 0 && (
                                <div className="space-y-3 animate-[fadeIn_0.2s_ease-out]">
                                    <div className="flex items-center gap-3">
                                        <label className="relative w-8 h-8 rounded-lg border border-line cursor-pointer overflow-hidden shrink-0" title="Background color">
                    ...
                    <button
                        ...
                        className="w-full py-3 mt-4 bg-primary hover:bg-[#2f2624] text-[#FBF8F4] font-bold rounded-xl shadow-lg shadow-primary/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2 shrink-0 disabled:opacity-50"
                    >
